...
from neuronal_network.loss.loc_loss import LocLoss, get_emitters_from_h5
...
class EmitterDataset(Dataset):
    ...
    def __init__(
        ...
        is_train: bool = True,
        pixel_size_nm: float = 100.0,  # 像素大小（纳米）
        z_range_nm: Tuple[float, float] = (-500.0, 500.0),  # z轴范围（纳米）
    ):
        """
        初始化数据集
    ...
    def _build_sample_index(self) -> List[Dict]:
        ...
        for tiff_idx, tiff_file in enumerate(self.tiff_files):
            ...
            try:
                ...
                # 加载所有帧的发射体信息
                with h5py.File(str(emitter_file), 'r') as f:
                    if 'records' in f and 'xyz' in f['records'] and 'phot' in f['records'] and 'frame_ix' in f['records']:
                        xyz = f['records/xyz'][()]
                        photons = f['records/phot'][()]
                        frame_indices = f['records/frame_ix'][()]
                    elif 'emitters' in f and 't0' in f['emitters'] and 'on_time' in f['emitters'] and 'xyz' in f['emitters']:
                        # 如果没有records组，但有emitters组，也可以使用
                        print(f"Using emitters group data from {emitter_file}")
                        # 这里需要额外处理，将emitters数据转换为每帧的记录
                        # 这部分代码需要根据实际情况调整
                        xyz = f['emitters/xyz'][()]
                        t0 = f['emitters/t0'][()]
                        ...
                        for i in range(len(t0)):
                            ...
                            end_frame = int(np.ceil(t0[i] + on_time[i]))
                            
                            for frame in range(start_frame, end_frame):
                                if frame < n_frames:  # 确保帧索引在有效范围内
                                    all_frame_indices.append(frame)
                                    all_xyz.append(xyz[i])
                                    all_photons.append(intensity[i])  # 使用强度作为光子数
                        
                        # 转换为numpy数组
                        frame_indices = np.array(all_frame_indices)
                        xyz = np.array(all_xyz)
                        photons = np.array(all_photons)
                    else:
                        print(f"Warning: Emitter file {emitter_file} missing required datasets, skipping")
                        continue
            ...
            for frame_idx in range(n_frames - self.frame_window + 1):
                ...
                # 获取中间帧的发射体位置信息
                frame_mask = frame_indices == middle_frame_idx
                frame_xyz = xyz[frame_mask] if np.any(frame_mask) else np.zeros((0, 3))
                frame_photons = photons[frame_mask] if np.any(frame_mask) else np.zeros(0)
                
                for y in y_positions:
                    for x in x_positions:
                        # 定义图像块区域
                        x_min, x_max = x, x + self.patch_size
                        y_min, y_max = y, y + self.patch_size
                        
                        # 筛选此图像块中的发射体
                        if len(frame_xyz) > 0:
                            # 转换发射体坐标到像素坐标（如果需要）
                            emitter_x = frame_xyz[:, 0] / self.pixel_size_nm if self.pixel_size_nm != 1.0 else frame_xyz[:, 0]
                            emitter_y = frame_xyz[:, 1] / self.pixel_size_nm if self.pixel_size_nm != 1.0 else frame_xyz[:, 1]
                            emitter_z = frame_xyz[:, 2]
                            
                            # 筛选在图像块内的发射体
                            in_patch = (emitter_x >= x_min) & (emitter_x < x_max) & \
                                      (emitter_y >= y_min) & (emitter_y < y_max)
                            
                            patch_xyz = frame_xyz[in_patch]
                            patch_photons = frame_photons[in_patch]
                            
                            # 计算此图像块中的发射体计数
                            patch_count = np.sum(in_patch)
                        else:
                            patch_xyz = np.zeros((0, 3))
                            patch_photons = np.zeros(0)
                            patch_count = 0
                        ...
                        samples.append({
                            ...
                            "x": x,
                            "y": y,
                            "count": patch_count,  # 使用精确的图像块内发射体计数
                            "tiff_idx": tiff_idx,
                            "emitters_xyz": patch_xyz,
                            "emitters_photons": patch_photons
                        })
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        # 确保帧数量固定
        while len(frames) < self.frame_window:
            # 添加零帧
            zero_frame = np.zeros((self.patch_size, self.patch_size), dtype=np.float32)
        ...
        # 处理发射体位置信息
        emitters_xyz = sample_info["emitters_xyz"]
        emitters_photons = sample_info["emitters_photons"]
        
        # 创建像素中心坐标网格
        x_center = sample_info["x"]
        y_center = sample_info["y"]
        z_center = 0.0  # 假设z中心为0
        
        # 创建像素中心坐标网格
        y_grid, x_grid = np.mgrid[0:self.patch_size, 0:self.patch_size]
        x_grid = x_grid + x_center
        y_grid = y_grid + y_center
        z_grid = np.zeros_like(x_grid) + z_center
        
        # 转换为张量
        pixel_centers_x = torch.from_numpy(x_grid.astype(np.float32)).unsqueeze(0)  # [1, H, W]
        pixel_centers_y = torch.from_numpy(y_grid.astype(np.float32)).unsqueeze(0)  # [1, H, W]
        pixel_centers_z = torch.from_numpy(z_grid.astype(np.float32)).unsqueeze(0)  # [1, H, W]
        
        # 处理发射体位置，转换为相对于图像块左上角的坐标
        if len(emitters_xyz) > 0:
            # 转换发射体坐标到像素坐标（如果需要）
            emitter_x = emitters_xyz[:, 0] / self.pixel_size_nm if self.pixel_size_nm != 1.0 else emitters_xyz[:, 0]
            emitter_y = emitters_xyz[:, 1] / self.pixel_size_nm if self.pixel_size_nm != 1.0 else emitters_xyz[:, 1]
            emitter_z = emitters_xyz[:, 2]
            
            # 转换为相对于图像块左上角的坐标
            emitter_x = emitter_x - x_center
            emitter_y = emitter_y - y_center
            
            # 组合为[E, 4]数组，包含[x, y, z, photons]
            emitters = np.column_stack([emitter_x, emitter_y, emitter_z, emitters_photons])
            emitters = torch.from_numpy(emitters.astype(np.float32))  # [E, 4]
        else:
            # 如果没有发射体，创建一个固定大小的零张量
            # 使用一个合理的最大发射体数量，例如50
            max_emitters = 50
            emitters = torch.zeros((max_emitters, 4), dtype=torch.float32)
        ...
        # 将frame_tensors列表转换为张量
        frames_tensor = torch.stack(frame_tensors, dim=0)  # [frame_window, 1, patch_size, patch_size]
        frames_tensor = frames_tensor.squeeze(1)  # [frame_window, patch_size, patch_size]
        
        return {
            "frames": frames_tensor,  # 形状为[frame_window, patch_size, patch_size]的张量
            "count": count,  # 标量
            "coords": (sample_info["x"], sample_info["y"]),  # 图像块坐标
            "frame_idx": sample_info["frame_idx"],  # 起始帧索引
            "tiff_idx": sample_info["tiff_idx"],  # TIFF文件索引
            "emitters": emitters,  # [E, 4] 发射体位置和光子数
            "pixel_centers_x": pixel_centers_x,  # [1, H, W] 像素中心x坐标
            "pixel_centers_y": pixel_centers_y,  # [1, H, W] 像素中心y坐标
            "pixel_centers_z": pixel_centers_z   # [1, H, W] 像素中心z坐标
        }
...
class TrainModel:
    ...
    def _init_data_loaders(self):
        ...
        def custom_collate_fn(batch):
            ...
            tiff_indices = [item["tiff_idx"] for item in batch]
            emitters_list = [item["emitters"] for item in batch]
            pixel_centers_x = [item["pixel_centers_x"] for item in batch]
            pixel_centers_y = [item["pixel_centers_y"] for item in batch]
            pixel_centers_z = [item["pixel_centers_z"] for item in batch]
            
            # 找到最大的发射体数量
            max_emitters = max([e.shape[0] for e in emitters_list])
            
            # 将所有发射体张量调整为相同大小
            padded_emitters = []
            for e in emitters_list:
                if e.shape[0] < max_emitters:
                    # 创建填充张量
                    padding = torch.zeros((max_emitters - e.shape[0], 4), dtype=e.dtype, device=e.device)
                    # 连接原始张量和填充张量
                    padded = torch.cat([e, padding], dim=0)
                    padded_emitters.append(padded)
                else:
                    padded_emitters.append(e)
            
            # 堆叠张量
            frames_batch = torch.stack(frames, dim=0)
            counts_batch = torch.stack(counts, dim=0)
            emitters_batch = torch.stack(padded_emitters, dim=0)
            pixel_centers_x_batch = torch.stack(pixel_centers_x, dim=0)
            pixel_centers_y_batch = torch.stack(pixel_centers_y, dim=0)
            pixel_centers_z_batch = torch.stack(pixel_centers_z, dim=0)
            ...
            return {
                ...
                "tiff_idx": tiff_indices,
                "emitters": emitters_batch,
                "pixel_centers_x": pixel_centers_x_batch,
                "pixel_centers_y": pixel_centers_y_batch,
                "pixel_centers_z": pixel_centers_z_batch
            }
    ...
    def _init_models(self):
        ...
        self.second_level_net = SecondLevelNet(
            ...
            with_loc_prediction=True  # 启用定位预测
        ).to(self.device)
    ...
    def _init_training_components(self):
        ...
        self.loc_loss_fn = LocLoss()
        self.background_loss_fn = BackgroundLoss()
        
        # 注意：损失权重已在__init__中设置
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            # 获取输入数据
            frames = batch["frames"].to(self.device)  # [B, frame_window, H, W]
            ...
            # 使用第二层网络处理特征
            with autocast(enabled=self.use_amp):
                outputs = self.second_level_net(concat_features)  # 字典或张量
                
                # 处理输出
                if isinstance(outputs, dict):
                    # 定位模式
                    probs = torch.sigmoid(outputs['prob'])  # [B, 1, H, W]
                    offsets = outputs['offset']  # [B, 3, H, W]
                    photons = outputs['photon']  # [B, 1, H, W]
                    uncertainties = outputs['uncertainty']  # [B, 4, H, W]
                    
                    # 获取像素中心坐标
                    pixel_centers_x = batch["pixel_centers_x"].to(self.device)  # [B, 1, H, W]
                    pixel_centers_y = batch["pixel_centers_y"].to(self.device)  # [B, 1, H, W]
                    pixel_centers_z = batch["pixel_centers_z"].to(self.device)  # [B, 1, H, W]
                    pixel_centers = torch.cat([pixel_centers_x, pixel_centers_y, pixel_centers_z], dim=1)  # [B, 3, H, W]
                    
                    # 获取发射体信息
                    emitters = batch["emitters"].to(self.device)  # [B, E, 4]
                    
                    # 计算计数损失
                    count_loss = self.count_loss_fn(probs, counts)
                    
                    # 计算定位损失
                    loc_loss = self.loc_loss_fn(
                        pred_detection=probs,
                        pred_dx=offsets[:, 0:1],  # x偏移
                        pred_dy=offsets[:, 1:2],  # y偏移
                        pred_dz=offsets[:, 2:3],  # z偏移
                        pred_photons=photons,  # 光子数
                        pred_sigma_x=uncertainties[:, 0:1],  # x不确定性
                        pred_sigma_y=uncertainties[:, 1:2],  # y不确定性
                        pred_sigma_z=uncertainties[:, 2:3],  # z不确定性
                        pred_sigma_photons=uncertainties[:, 3:4],  # 光子数不确定性
                        gt_emitters=emitters,  # 真实发射体
                        pixel_centers_x=pixel_centers_x,  # 像素中心x坐标
                        pixel_centers_y=pixel_centers_y,  # 像素中心y坐标
                        pixel_centers_z=pixel_centers_z   # 像素中心z坐标
                    )
                    
                    # 计算背景损失
                    # 使用第一帧作为真实背景图
                    true_background = frames[:, 0:1]  # [B, 1, H, W]
                    background = outputs['background']  # [B, 1, H, W]
                    background_loss = self.background_loss_fn(background, true_background)
                    
                    # 计算加权损失
                    weighted_count_loss = self.count_loss_weight * count_loss
                    weighted_loc_loss = self.loc_loss_weight * loc_loss
                    weighted_background_loss = self.background_loss_weight * background_loss
                    
                    # 组合损失
                    loss = weighted_count_loss + weighted_loc_loss + weighted_background_loss
                    
                    # 保存各个损失组件，用于TensorBoard记录
                    self.current_losses = {
                        'count_loss': count_loss.item(),
                        'loc_loss': loc_loss.item(),
                        'background_loss': background_loss.item(),
                        'weighted_count_loss': weighted_count_loss.item(),
                        'weighted_loc_loss': weighted_loc_loss.item(),
                        'weighted_background_loss': weighted_background_loss.item()
                    }
                else:
                    # 仅计数模式
                    logits = outputs  # [B, 1, H, W]
                ...
                with autocast(enabled=self.use_amp):
                    ...
                    if isinstance(outputs, dict):
                        ...
                        loc_loss = self.loc_loss_fn(
                            ...
                            pred_dy=offsets[:, 1:2],
                            pred_dz=offsets[:, 2:3],
                            pred_photons=photons,
                            pred_sigma_x=uncertainties[:, 0:1],
                            pred_sigma_y=uncertainties[:, 1:2],
                            pred_sigma_z=uncertainties[:, 2:3],
                            pred_sigma_photons=uncertainties[:, 3:4],
                            gt_emitters=emitters,
                            pixel_centers_x=pixel_centers_x,
                            pixel_centers_y=pixel_centers_y,
                            pixel_centers_z=pixel_centers_z
                        )
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if hasattr(self, 'val_losses'):
                for loss_name, loss_value in self.val_losses.items():
...
def main():
    ...
    parser.add_argument("--emitter_dir", type=str, default="/home/guest/Others/DECODE_rewrite/simulated_data_multi_frames/emitter_sets",
                        help="发射体H5文件目录")
    parser.add_argument("--output_dir", type=str, default="/home/guest/Others/DECODE_rewrite/nn_train_loc_back",
                        help="输出目录")
