...
# 修改：使用count_loss_fix中的CountLoss
from neuronal_network.loss.count_loss_fix import CountLoss, get_emitter_count_per_frame
...
class MemoryTracker:
    ...
    def __init__(self, log_dir: str, enabled: bool = True):
        ...
        self.log_file = os.path.join(log_dir, "memory_usage.csv")
        
        if self.enabled:
            # 创建日志文件
            with open(self.log_file, "w") as f:
                f.write("timestamp,allocated_memory_mb,reserved_memory_mb\n")
    ...
    def update(self):
        ...
        with open(self.log_file, "a") as f:
            f.write(f"{timestamp},{allocated:.2f},{reserved:.2f}\n")
...
class TrainModel:
    ...
    def _init_models(self):
        ...
        self.second_level_net = SecondLevelNet(
            ...
            with_loc_prediction=False  # 设置为False，只返回检测概率
        ).to(self.device)
    ...
    def _init_training_components(self):
        ...
        # 初始化损失函数 - 使用count_loss_fix中的CountLoss
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            # 使用第二层网络处理特征
            with autocast('cuda', enabled=self.use_amp):
                logits = self.second_level_net(concat_features)  # [B, 1, H, W] 或字典
                
                # 处理可能的字典返回值
                if isinstance(logits, dict):
                    prob_logits = logits['prob']
                else:
                    prob_logits = logits
                    
                probs = torch.sigmoid(prob_logits)  # [B, 1, H, W]
